...
/*
 * sfs_block_free - set related bits for ino block to 1(means free) in bitmap,
 * add sfs->super.unused_blocks, set superblock dirty *
 */
...
/*
 * sfs_create_inode - alloc a inode in memroy, and init
 * din/ino/dirty/reclian_count/sem fields in sfs_inode in inode
 */
...
/*
 * sfs_load_inode - If the inode isn't existed, load inode related ino disk
 * block data into a new created inode. If the inode is in memory alreadily,
 * then do nothing
 */
int sfs_load_inode(struct sfs_fs* sfs, struct inode** node_store,
                   uint32_t ino) {
  lock_sfs_fs(sfs);
  struct inode* node;
...
/*
 * sfs_bmap_get_sub_nolock - according entry pointer entp and index, find the
 * index of indrect disk block return the index of indrect disk block to
 * ino_store. no lock protect
 * @sfs:      sfs file system
 * @entp:     the pointer of index of entry disk block
 * @index:    the index of block in indrect block
 * @create:   BOOL, if the block isn't allocated, if create = 1 the alloc a
 * block,  otherwise just do nothing
...
static int sfs_bmap_get_sub_nolock(struct sfs_fs* sfs, uint32_t* entp,
  ...
  // if entry block is existd, read the content of entry block into
  // sfs->sfs_buffer
  if ((ent = *entp) != 0) {
...
/*
 * sfs_bmap_get_nolock - according sfs_inode and index of block, find the NO. of
 * disk block no lock protect
 * @sfs:      sfs file system
...
/*
 * sfs_bmap_free_sub_nolock - set the entry item to 0 (free) in the indirect
 * block
 */
...
/*
 * sfs_bmap_free_nolock - free a block with logical index in inode and reset the
 * inode's fields
 */
...
/*
 * sfs_bmap_load_nolock - according to the DIR's inode and the logical index of
 * block in inode, find the NO. of disk block.
 * @sfs:      sfs file system
 * @sin:      sfs inode in memory
...
/*
 * sfs_dirent_read_nolock - read the file entry from disk block which contains
 * this entry
 * @sfs:      sfs file system
 * @sin:      sfs inode in memory
...
static int sfs_dirent_read_nolock(struct sfs_fs* sfs, struct sfs_inode* sin,
  ...
  int      ret;
  uint32_t ino;
  // according to the DIR's inode and the slot of file entry, find the index of
  // disk block which contains this file entry
...
/*
 * sfs_dirent_search_nolock - read every file entry in the DIR, compare file
 * name with each entry->name If equal, then return slot and NO. of disk of this
 * file's inode
 * @sfs:        sfs file system
 * @sin:        sfs inode in memory
 * @name:       the filename
 * @ino_store:  NO. of disk of this file (with the filename)'s inode
 * @slot:       logical index of file entry (NOTICE: each file entry ocupied one
 * disk block)
...
/*
 * sfs_dirent_findino_nolock - read all file entries in DIR's inode and find a
 * entry->ino == ino
 */
...
static int sfs_lookup_once(struct sfs_fs* sfs, struct sfs_inode* sin,
                           const char* name, struct inode** node_store,
                           int* slot) {
  int      ret;
  uint32_t ino;
  lock_sin(sin);
...
/*
 * sfs_io_nolock - Rd/Wr a file contentfrom offset position to offset+ length
...
static int sfs_io_nolock(struct sfs_fs* sfs, struct sfs_inode* sin, void* buf,
  ...
  *alenp       = 0;
   ...
   * function: sfs_bmap_load_nolock, sfs_block_op (3) If end position isn't
   * aligned with the last block, Rd/Wr some content from begin to the (endpos %
   * SFS_BLKSIZE) of the last block NOTICE: useful function:
   * sfs_bmap_load_nolock, sfs_buf_op
   */
...
/*
 * sfs_fstat - Return nlinks/block/size, etc. info about a file. The pointer is
...
/*
 * sfs_fsync - Force any dirty inode info associated with this file to stable
 * storage.
 */
static int sfs_fsync(struct inode* node) {
  ...
  if (sin->dirty) {
    lock_sin(sin);
    {
      if (sin->dirty) {
        sin->dirty = 0;
        if ((ret = sfs_wbuf(sfs, sin->din, sizeof(struct sfs_disk_inode),
                            sin->ino, 0))
            != 0) {
          sin->dirty = 1;
        }
...
/*
 *sfs_namefile -Compute pathname relative to filesystem root of the file and
 *copy to the specified io buffer.
 *
 */
...
/*
 * sfs_getdirentry - according to the iob->io_offset, calculate the dir entry's
 slot in disk block, get dir entry content from the disk
 */
...
/*
 * sfs_reclaim - Free all resources inode occupied . Called when inode is no
 * longer in use.
 */
...
/*
 * sfs_tryseek - Check if seeking to the specified position within the file is
 * legal.
 */
 ...
 *              refers to.
 */
static int sfs_lookup(struct inode* node, char* path,
...
// The sfs specific DIR operations correspond to the abstract operations on a
// inode.
...
/// The sfs specific FILE operations correspond to the abstract operations on a
/// inode.
