...
/*
 * vfs_get_devname - Given a filesystem, hand back the name of the device it's
 * mounted on.
 */
 ...
 * for direct access.
 */
static int vfs_do_add(const char* devname, struct inode* devnode, struct fs* fs,
                      bool mountable) {
  assert(devname != NULL);
...
/*
 * vfs_add_fs - Add a new fs,  by name. See  vfs_do_add information for the
 * description of mountable.
 */
...
/*
 * vfs_add_dev - Add a new device, by name. See  vfs_do_add information for the
 * description of mountable.
 */
...
/*
 * vfs_mount - Mount a filesystem. Once we've found the device, call MOUNTFUNC
 * to set up the filesystem and hand back a struct fs.
 *
 ...
 *               First calls FSOP_SYNC on the filesystem; then calls
 * FSOP_UNMOUNT.
 */
