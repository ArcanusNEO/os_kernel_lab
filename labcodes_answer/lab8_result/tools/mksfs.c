...
struct sfs_fs* create_sfs(int imgfd) {
  ...
  if ((next_ino = SFS_BLKN_FREEMAP + (ninos + SFS_BLKBITS - 1) / SFS_BLKBITS)
      >= ninos) {
    bug("img file is too small (%llu bytes, %u blocks, bitmap use at least %u "
        "blocks).\n",
...
static void write_block(struct sfs_fs* sfs, void* data, size_t len,
                        uint32_t ino) {
...
struct sfs_fs* open_img(const char* imgname) {
  const char *expect = ".img",
             *ext    = imgname + strlen(imgname) - strlen(expect);
...
static void update_cache(struct sfs_fs* sfs, struct cache_block** cbp,
                         uint32_t* inop) {
  uint32_t            ino = *inop;
...
static void append_block(struct sfs_fs* sfs, struct cache_inode* file,
                         size_t size, uint32_t ino, const char* filename) {
...
static void add_entry(struct sfs_fs* sfs, struct cache_inode* current,
...
static void add_dir(struct sfs_fs* sfs, struct cache_inode* parent,
                    const char* dirname, int curfd, int fd, ino_t real) {
...
static void add_file(struct sfs_fs* sfs, struct cache_inode* current,
                     const char* filename, int fd, ino_t real) {
...
static void add_link(struct sfs_fs* sfs, struct cache_inode* current,
                     const char* filename, ino_t real) {
...
#  error Unsupported architecture
  ...
  static_assert(SFS_MAX_FILE_SIZE <= 0x80000000UL,
                "SFS_MAX_FILE_SIZE <= 0x80000000UL");
}
