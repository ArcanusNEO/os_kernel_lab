...
rb_tree* rb_tree_create(int (*compare)(rb_node* node1, rb_node* node2)) {
  ...
  if ((tree = kmalloc(sizeof(rb_tree))) == NULL) { goto bad_tree; }

  tree->compare = compare;

  if ((nil = rb_node_create()) == NULL) { goto bad_node_cleanup_tree; }

  nil->parent = nil->left = nil->right = nil;
  nil->red                             = 0;
  tree->nil                            = nil;

  if ((root = rb_node_create()) == NULL) { goto bad_node_cleanup_nil; }
...
void rb_delete(rb_tree* tree, rb_node* node) {
  ...
  if (need_fixup) { rb_delete_fixup(tree, x); }
}
...
int check_tree(rb_tree* tree, rb_node* node) {
  ...
  if (node->red) { assert(!node->left->red && !node->right->red); }
...
void check_rb_tree(void) {
  ...
  for (i = 0; i < total; i++) { mark[all[i]->data] = 1; }
  for (i = 0; i < total; i++) { assert(mark[i] == 1); }
