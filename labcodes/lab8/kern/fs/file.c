...
// fs_array_alloc - allocate a free file item (with FD_NONE status) in open
// files table
...
// fd_array_release - file's open_count--; if file's open_count-- == 0 , then
// call fd_array_free to free this file item
...
// fd_array_close - file's open_count--; if file's open_count-- == 0 , then call
// fd_array_free to free this file item
...
int file_getdirentry(int fd, struct dirent* direntp) {
  ...
  struct iobuf __iob, *iob = iobuf_init(&__iob, direntp->name,
                                        sizeof(direntp->name), direntp->offset);
