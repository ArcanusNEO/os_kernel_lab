...
int sysfile_open(
  const char* path,
  uint32_t open_flags);  // Open or create a file. FLAGS/MODE per the syscall.
...
int sysfile_link(const char* path1,
                 const char* path2);  // set a path1's link as path2
...
int sysfile_getdirentry(int            fd,
                        struct dirent* direntp);  // get the file entry in DIR
