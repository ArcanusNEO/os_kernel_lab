...
#define vop_gettype(node, type_store) \
  (__vop_op(node, gettype)(node, type_store))
...
#define vop_lookup(node, path, node_store) \
  (__vop_op(node, lookup)(node, path, node_store))
