...
/*
 * sfs_unmount - unmount sfs, and free the memorys contain
 * sfs->freemap/sfs_buffer/hash_liskt and sfs itself.
 */
...
/*
 * sfs_cleanup - when sfs failed, then should call this function to sync sfs by
 * calling sfs_sync
 *
 * NOTICE: nouse now.
 */
...
static int sfs_init_read(struct device* dev, uint32_t blkno, void* blk_buffer) {
  struct iobuf __iob,
    *iob = iobuf_init(&__iob, blk_buffer, SFS_BLKSIZE, blkno * SFS_BLKSIZE);
  return dop_io(dev, iob, 0);
}

/*
 * sfs_init_freemap - used in sfs_do_mount to read freemap data info in disk
 * block(blkno, nblks) directly.
 *
 * @dev:        the block device
 ...
 *      (2) read dev into iobuf
 */
static int sfs_init_freemap(struct device* dev, struct bitmap* freemap,
                            uint32_t blkno, uint32_t nblks, void* blk_buffer) {
  size_t len;
...
static int sfs_do_mount(struct device* dev, struct fs** fs_store) {
  ...
  if ((sfs->hash_list = hash_list =
         kmalloc(sizeof(list_entry_t) * SFS_HLIST_SIZE))
      == NULL) {
    goto failed_cleanup_sfs_buffer;
  }
  ...
  if ((ret = sfs_init_freemap(dev, freemap, SFS_BLKN_FREEMAP,
                              freemap_size_nblks, sfs_buffer))
      != 0) {
  ...
  /* link addr of sync/get_root/unmount/cleanup funciton  fs's function
   * pointers*/
  fs->fs_sync     = sfs_sync;
