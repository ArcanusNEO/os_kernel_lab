 ...
 *                      without lock protect for mutex process on Rd/Wr disk
 * block
 ...
 */
static int sfs_rwblock_nolock(struct sfs_fs* sfs, void* buf, uint32_t blkno,
                              bool write, bool check) {
  assert((blkno != 0 || !check) && blkno < sfs->super.blocks);
  struct iobuf __iob,
    *iob = iobuf_init(&__iob, buf, SFS_BLKSIZE, blkno * SFS_BLKSIZE);
 ...
 */
static int sfs_rwblock(struct sfs_fs* sfs, void* buf, uint32_t blkno,
                       uint32_t nblks, bool write) {
  int ret = 0;
  lock_sfs_io(sfs);
  {
    while (nblks != 0) {
...
/* sfs_rbuf - The Basic block-level I/O routine for  Rd( non-block & non-aligned
 * io) one disk block(using sfs->sfs_buffer) with lock protect for mutex process
 * on Rd/Wr disk block
 * @sfs:    sfs_fs which will be process
...
/* sfs_wbuf - The Basic block-level I/O routine for  Wr( non-block & non-aligned
 * io) one disk block(using sfs->sfs_buffer) with lock protect for mutex process
 * on Rd/Wr disk block
 * @sfs:    sfs_fs which will be process
...
/*
 * sfs_sync_super - write sfs->super (in memory) into disk (SFS_BLKN_SUPER, 1)
 * with lock protect.
 */
...
/*
 * sfs_sync_freemap - write sfs bitmap into disk (SFS_BLKN_FREEMAP, nblks)
 * without lock protect.
 */
int sfs_sync_freemap(struct sfs_fs* sfs) {
  uint32_t nblks = sfs_freemap_blocks(&(sfs->super));
  return sfs_wblock(sfs, bitmap_getdata(sfs->freemap, NULL), SFS_BLKN_FREEMAP,
                    nblks);
}

/*
 * sfs_clear_block - write zero info into disk (blkno, nblks)  with lock
 * protect.
 * @sfs:   sfs_fs which will be process
 * @blkno: the NO. of disk block
 * @nblks: Rd/Wr number of disk block
